...
class SiteController extends Controller {
    ...
    public function init() {
        ...
        $listProvinceEast = Province::model()->findAllByAttributes(array('reg_id' => 3), array('order' => 'pro_name_th'));
        ...
        if (intval(strpos($baseDomain, 'pranew'))) {
            if (intval(strpos($baseUrl, 'upload'))) {
                if (!intval(strpos($baseDomain, 'www'))) {
                    header("Location: http://www.pranew.com/site/upload");
                    exit();
                }
            }
        }
    ...
    public function actionRenderHtmlSacred() {
        ...
        if (!empty(Yii::app()->session['criteria'])) {
            ...
            $criteriaUser = $criterias['user'];
            $criteriaRegion = $criterias['region'];
            $criteriaForm = $criterias['form'];
            if (!empty($criteriaUser)) {
                $where_condition .= 'AND mem_id = ' . $criteriaUser;
                $criteria->compare('o.mem_id', $criteriaUser, true);
            }
            if (!empty($criteriaForm['freeSearch'])) {
                $criteria->compare('o.obj_name', $criteriaForm['freeSearch'], true);
                $where_condition .= " AND o.obj_name like '%" . $criteriaForm['freeSearch'] . "%' ";
            }
         ...
         * pagiation logic
         */
        $this->renderPartial('load_listSacreds', $this->data);
    }
    ...
    public function actionIndex() {
         ...
         * Meta SEO Tag
         */
        $this->pageTitle = 'www.pranew.com เว็บพระเครื่องของคนรุ่นใหม่ ซื้อ ขาย พระเครื่อง วัตถุมงคล ฯลฯ ฟรี 
...
//exit();
         ...
         * Meta SEO tag
         */
       //var_dump($this->data['sacredObject']);
        //exit();
    ...
    public function actionUpload($id = null) {
        if (empty($this->member->mem_id)) {
            ...
            if (empty($_POST)) {
                ...
                $listRegion = Region::model()->findAllByPk(3);
